...
class FakeDB {
  ...
  findAllFavorites() {
    const response = {
      artists: db.artists.filter(
        (artist) => this.favorites.artists.indexOf(artist.id) !== -1,
      ),
      albums: db.albums.filter(
        (album) => this.favorites.albums.indexOf(album.id) !== -1,
      ),
      tracks: db.tracks.filter(
        (track) => this.favorites.tracks.indexOf(track.id) !== -1,
      ),
    };
    return response;
  }
